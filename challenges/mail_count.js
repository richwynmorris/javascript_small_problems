...
function mailCount(emailData) {
  let emailCollection = removeNewLines(emailData).split('##||##')
  let organizedData = emailCollection.map(emailData => {
    let components = emailData.split('#/#')
      return {
        sender: components[0].replace('From: ', ''),
        subject: components[1].replace('Subject: ', ''),
        date: components[2].replace('Date: ', ''),
        recipient: components[3].replace('To: ', ''),
        body: components[4],
      }
  })
  
  let dates = organizedData.map(data => data.date).sort()
  let firstDate = new Date(dates[0]).toDateString()
  let lastDate = new Date(dates[dates.length - 1]).toDateString()
  
  console.log(`Count of Email: ${organizedData.length}`)
  console.log(`Date Rage: ${firstDate} - ${lastDate}`)
}
